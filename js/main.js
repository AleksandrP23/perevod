...
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _components_faq_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./components/faq.js */ "./src/js/components/faq.js");
/* harmony import */ var _components_hero_calculator_js__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./components/hero-calculator.js */ "./src/js/components/hero-calculator.js");
/* harmony import */ var _components_header_dropdown_js__WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./components/header-dropdown.js */ "./src/js/components/header-dropdown.js");
/* harmony import */ var _components_sliders_resize_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ./components/sliders-resize.js */ "./src/js/components/sliders-resize.js");
/* harmony import */ var _functions_burger_js__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(/*! ./functions/burger.js */ "./src/js/functions/burger.js");
...
if (faqRoot) {
  const questions = faqRoot.querySelectorAll('.faq__question');
...
dropdownRoots.forEach(root => {
  ...
  const phoneLinks = document.querySelectorAll('[data-header-phone]');
  const mailLinks = document.querySelectorAll('[data-header-mail]');
  root.querySelectorAll('.header__office-option[data-phone-href]').forEach(opt => {
    opt.addEventListener('click', () => {
      ...
      phoneLinks.forEach(phoneLink => {
        phoneLink.href = opt.dataset.phoneHref || phoneLink.href;
        const span = phoneLink.querySelector('.header__contact-text');
        if (span && opt.dataset.phoneText) {
          span.textContent = opt.dataset.phoneText;
        }
      });
      mailLinks.forEach(mailLink => {
        mailLink.href = opt.dataset.mailHref || mailLink.href;
        const span = mailLink.querySelector('.header__contact-text');
        if (span && opt.dataset.mailText) {
          span.textContent = opt.dataset.mailText;
        }
      });
      close();
    });
...
const desktopSubmenuBtn = document.querySelector('[data-header-submenu-btn]');
const desktopSubmenu = document.getElementById('header-passport-submenu');
const showAllBtn = document.querySelector('[data-header-show-all]');
const desktopSubmenuOverlay = document.querySelector('[data-header-submenu-overlay]');
const headerRoot = document.querySelector('.header');
function syncHeaderScrolledState() {
  if (!headerRoot) {
    return;
  }
  headerRoot.classList.toggle('header--scrolled', window.scrollY > 4);
}
syncHeaderScrolledState();
window.addEventListener('scroll', syncHeaderScrolledState, {
  passive: true
});
function setDesktopSubmenuOpen(isOpen) {
  if (!desktopSubmenuBtn || !desktopSubmenu || !showAllBtn) {
    return;
  }
  desktopSubmenuBtn.setAttribute('aria-expanded', String(isOpen));
  showAllBtn.setAttribute('aria-expanded', String(isOpen));
  showAllBtn.setAttribute('aria-label', isOpen ? 'Скрыть меню' : 'Показать все');
  desktopSubmenu.hidden = !isOpen;
  if (desktopSubmenuOverlay) {
    desktopSubmenuOverlay.hidden = !isOpen;
  }
}
desktopSubmenuBtn?.addEventListener('click', () => {
  const isOpen = desktopSubmenuBtn.getAttribute('aria-expanded') === 'true';
  setDesktopSubmenuOpen(!isOpen);
});
showAllBtn?.addEventListener('click', () => {
  const isOpen = showAllBtn.getAttribute('aria-expanded') === 'true';
  setDesktopSubmenuOpen(!isOpen);
});
desktopSubmenuOverlay?.addEventListener('click', () => {
...
document.addEventListener('click', event => {
  ...
  if (desktopSubmenu && !desktopSubmenu.contains(target) && !desktopSubmenuBtn?.contains(target) && !showAllBtn?.contains(target)) {
    setDesktopSubmenuOpen(false);
  }
});
...
const languageCurrentLabels = document.querySelectorAll('[data-header-lang-current]');
...
function applyLanguage(lang) {
  ...
  languageCurrentLabels.forEach(label => {
    label.textContent = normalized;
  });
  languageToggleButtons.forEach(button => {
    button.dataset.langValue = value;
    button.setAttribute('aria-label', isEnglish ? 'Сменить язык на русский' : 'Switch language to English');
  });
  languageFlagImages.forEach(img => {
    img.src = isEnglish ? 'img/en_flag.png' : 'img/ru_flag.png';
  });
  window.localStorage.setItem(languageStorageKey, value);
}
const storedLanguage = window.localStorage.getItem(languageStorageKey);
applyLanguage(storedLanguage || 'ru');
languageToggleButtons.forEach(button => {
  button.addEventListener('click', () => {
    const nextLang = button.dataset.langValue === 'en' ? 'ru' : 'en';
...
/***/ "./src/js/components/hero-calculator.js"
/*!**********************************************!*\
  !*** ./src/js/components/hero-calculator.js ***!
  \**********************************************/
(__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) {
...
function closeHeroSelect(root) {
  const trigger = root.querySelector('.hero__select-trigger');
  const panel = root.querySelector('.hero__select-panel');
  root.classList.remove('is-open');
...
const selectRoots = document.querySelectorAll('[data-hero-select]');
selectRoots.forEach(selectRoot => {
  const trigger = selectRoot.querySelector('.hero__select-trigger');
  const panel = selectRoot.querySelector('.hero__select-panel');
  const hiddenInput = selectRoot.querySelector('input[type="hidden"]');
  const valueEl = selectRoot.querySelector('.hero__select-value');
  trigger?.addEventListener('click', e => {
    ...
    if (panel.hidden) {
      selectRoots.forEach(root => {
        if (root !== selectRoot) {
          closeHeroSelect(root);
        }
      });
      openHeroSelect(selectRoot);
    } else {
      closeHeroSelect(selectRoot);
    }
  });
  selectRoot.querySelectorAll('.hero__select-option').forEach(opt => {
    opt.addEventListener('click', () => {
      if (valueEl) {
...
const graphModal = new graph_modal__WEBPACK_IMPORTED_MODULE_0__["default"]();
document.querySelectorAll('.calc-modal__field--file input[type="file"]').forEach(fileInput => {
  const field = fileInput.closest('.calc-modal__field--file');
  const filePlaceholder = field?.querySelector('.calc-modal__file-placeholder');
  fileInput.addEventListener('change', () => {
    const file = fileInput.files?.[0];
    if (filePlaceholder) {
      filePlaceholder.textContent = file ? file.name : 'Выберите файл';
    }
  });
});
document.querySelectorAll('.calc-modal__form').forEach(form => {
...
/***/ "./src/js/components/sliders-resize.js"
/*!*********************************************!*\
  !*** ./src/js/components/sliders-resize.js ***!
  \*********************************************/
(__unused_webpack___webpack_module__, __webpack_exports__, __webpack_require__) {
...
const BP = '(max-width: 576px)';

/**
 * Выравнивает высоту всех слайдов по самому высокому (мобильный режим).
 */
function equalizeSwiperSlidesHeights(swiper) {
...
function clearSlideHeights(selector) {
  document.querySelectorAll(`${selector} .swiper-slide`).forEach(slide => {
    slide.style.minHeight = '';
  });
}
function resizableSwiper(selector, settings) {
  let swiper;
  ...
  const apply = () => {
    ...
    if (mq.matches) {
      if (swiper) {
        swiper.update();
        equalizeSwiperSlidesHeights(swiper);
        return;
      }
      swiper = new swiper__WEBPACK_IMPORTED_MODULE_0__["default"](selector, {
        ...
        on: {
          init(sw) {
            equalizeSwiperSlidesHeights(sw);
          },
          resize(sw) {
            equalizeSwiperSlidesHeights(sw);
          }
...
resizableSwiper('.js-services-swiper', {
  slidesPerView: 1,
  spaceBetween: 10,
  navigation: {
...
resizableSwiper('.js-how-work-swiper', {
  slidesPerView: 1,
  spaceBetween: 20,
  navigation: {
    nextEl: '.js-how-work-next',
    prevEl: '.js-how-work-prev'
  }
});
